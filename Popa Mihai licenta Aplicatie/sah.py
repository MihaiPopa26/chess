...
def menu_screen(win, name):
    ...
    while run:
        ...
        if offline:
            off = small_font.render("Server Offline", 1, (255, 0, 0))
...
def redraw_gameWindow(win, bo, p1, p2, color, ready):
    ...
    try:
        txt = font.render(bo.p1Name + "\'timp: " + str(formatTime2), 1, (255, 255, 255))
        txt2 = font.render(bo.p2Name + "\'timp: " + str(formatTime1), 1, (255,255,255))
    except Exception as e:
        print(e)
    win.blit(txt, (520,10))
    win.blit(txt2, (520, 700))

    txt = font.render("Q pentru a parasi", 1, (255, 255, 255))
    win.blit(txt, (10, 20))
    ...
    if not color == "s":
        font = pygame.font.SysFont("comicsans", 30)
        if color == "w":
            txt3 = font.render("Alb", 1, (255, 0, 0))
            win.blit(txt3, (width / 2 - txt3.get_width() / 2, 10))
        else:
            txt3 = font.render("Negru", 1, (255, 0, 0))
            win.blit(txt3, (width / 2 - txt3.get_width() / 2, 10))

        if bo.turn == color:
            txt3 = font.render("Randul tau", 1, (255, 0, 0))
            win.blit(txt3, (width / 2 - txt3.get_width() / 2, 700))
        else:
            txt3 = font.render("Randul lor", 1, (255, 0, 0))
            win.blit(txt3, (width / 2 - txt3.get_width() / 2, 700))
...
def main():
    ...
    while run:
        ...
        try:
            ...
            print(e)
            end_screen(win, "A parasit jocul")
            run = False
            break
        ...
        if bo.winner == "w":
            end_screen(win, "Alb a castigat")
            run = False
        elif bo.winner == "b":
            end_screen(win, "Negru a castigat")
            run = False
